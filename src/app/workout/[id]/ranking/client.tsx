...
function sortParticipants(list: any[]): any[] {
  return [...list].sort((a, b) => {
    ...
    // Both dnf → sort by elapsed_ms (all same = time cap)
            ...
            <div
              ...
              className={`flex items-center gap-4 rounded-2xl px-5 py-4 transition-all ${
                isDNF
                  ...
                  : position === 3
                  ? 'bg-gradient-to-r from-orange-700/15 to-transparent border border-orange-700/30'
                  : 'bg-white/5'
              }`}
            >
              {/* Position / Medal */}
...
export function WorkoutRankingClient({
  workoutId,
  initialParticipants,
  timeCap,
  sessionIds,
  ...
  useEffect(() => {
    ...
    // Escuta mudanças de resultados e participantes nas sessões deste treino
    const channels = sessionIds.map((sessionId) => {
      return supabase
        .channel(`workout_ranking_${sessionId}`)
        .on(
          'postgres_changes',
  ...
  return (
    <div className="flex-1 max-w-lg mx-auto w-full pb-12">
      ...
      <p className="text-center text-xs text-white/20 mt-8">Round Count • Ranking Consolidado</p>
    </div>
  )
}
